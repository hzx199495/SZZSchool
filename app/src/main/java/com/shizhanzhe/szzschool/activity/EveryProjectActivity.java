...
import com.shizhanzhe.szzschool.video.PolyvPlayerAuditionView;
import com.shizhanzhe.szzschool.video.PolyvPlayerAuxiliaryView;
import com.shizhanzhe.szzschool.video.PolyvPlayerDanmuFragment;
import com.shizhanzhe.szzschool.video.PolyvPlayerEndFragment;
import com.shizhanzhe.szzschool.video.PolyvPlayerLightView;
import com.shizhanzhe.szzschool.video.PolyvPlayerMediaController;
import com.shizhanzhe.szzschool.video.PolyvPlayerPreviewView;
import com.shizhanzhe.szzschool.video.PolyvPlayerProgressView;
import com.shizhanzhe.szzschool.video.PolyvPlayerQuestionView;
import com.shizhanzhe.szzschool.video.PolyvPlayerTopFragment;
import com.shizhanzhe.szzschool.video.PolyvPlayerVolumeView;
 ...
 */
public class EveryProjectActivity extends FragmentActivity {
    private static final String TAG = EveryProjectActivity.class.getSimpleName();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_everyproject);
        ...
        findIdAndNew();
        initView();
        int playModeCode = getIntent().getIntExtra("playMode", EveryProjectActivity.PlayMode.portrait.getCode());
        EveryProjectActivity.PlayMode playMode = EveryProjectActivity.PlayMode.getPlayMode(playModeCode);
        if (playMode == null)
            playMode = EveryProjectActivity.PlayMode.portrait;
...
//        ft.add(R.id.fl_viewpager, viewPagerFragment, "viewPagerFragment");
    ...
    private void findIdAndNew() {
        ...
        TextView protitle = findViewById(R.id.protitle);
        TextView prointroduce = findViewById(R.id.prointroduce);
        try {
            JSONObject object = new JSONObject(MyApplication.every);
            protitle.setText(object.optString("title"));
            prointroduce.setText(object.optString("descri"));
    ...
    private void initView() {
        ...
        videoView.setOnVideoStatusListener(new IPolyvOnVideoStatusListener() {
            ...
            public void onStatus(int status) {
                if (status < 60) {
                    Toast.makeText(EveryProjectActivity.this, String.format("状态错误 %d", status), Toast.LENGTH_SHORT).show();
                } else {
            ...
            public boolean onVideoPlayError(@PolyvPlayErrorReason.PlayErrorReason int playErrorReason) {
                switch (playErrorReason) {
                    case PolyvPlayErrorReason.NETWORK_DENIED:
                        Toast.makeText(EveryProjectActivity.this, "无法连接网络，请连接网络后播放", Toast.LENGTH_LONG).show();
                        break;
                    case PolyvPlayErrorReason.OUT_FLOW:
                        Toast.makeText(EveryProjectActivity.this, "流量超标(error code " + PolyvPlayErrorReason.OUT_FLOW + ")", Toast.LENGTH_LONG).show();
                        break;
                    case PolyvPlayErrorReason.TIMEOUT_FLOW:
                        Toast.makeText(EveryProjectActivity.this, "账号过期(error code " + PolyvPlayErrorReason.TIMEOUT_FLOW + ")", Toast.LENGTH_LONG).show();
                        break;
                    case PolyvPlayErrorReason.LOCAL_VIEWO_ERROR:
                        Toast.makeText(EveryProjectActivity.this, "本地视频文件损坏，请重新下载(error code " + PolyvPlayErrorReason.LOCAL_VIEWO_ERROR + ")", Toast.LENGTH_LONG).show();
                        break;
                    case PolyvPlayErrorReason.START_ERROR:
                        Toast.makeText(EveryProjectActivity.this, "播放异常，请重新播放(error code " + PolyvPlayErrorReason.START_ERROR + ")", Toast.LENGTH_LONG).show();
                        break;
                    case PolyvPlayErrorReason.NOT_PERMISSION:
                        Toast.makeText(EveryProjectActivity.this, "非法播放(error code " + PolyvPlayErrorReason.NOT_PERMISSION + ")", Toast.LENGTH_LONG).show();
                        break;
                    case PolyvPlayErrorReason.USER_TOKEN_ERROR:
                        Toast.makeText(EveryProjectActivity.this, "请先设置播放凭证，再进行播放(error code " + PolyvPlayErrorReason.USER_TOKEN_ERROR + ")", Toast.LENGTH_LONG).show();
                        break;
                    case PolyvPlayErrorReason.VIDEO_STATUS_ERROR:
                        Toast.makeText(EveryProjectActivity.this, "视频状态异常，无法播放(error code " + PolyvPlayErrorReason.VIDEO_STATUS_ERROR + ")", Toast.LENGTH_LONG).show();
                        break;
                    case PolyvPlayErrorReason.VID_ERROR:
                        Toast.makeText(EveryProjectActivity.this, "视频id不正确，请设置正确的视频id进行播放(error code " + PolyvPlayErrorReason.VID_ERROR + ")", Toast.LENGTH_LONG).show();
                        break;
                    case PolyvPlayErrorReason.BITRATE_ERROR:
                        Toast.makeText(EveryProjectActivity.this, "清晰度不正确，请设置正确的清晰度进行播放(error code " + PolyvPlayErrorReason.BITRATE_ERROR + ")", Toast.LENGTH_LONG).show();
                        break;
                    case PolyvPlayErrorReason.VIDEO_NULL:
                        Toast.makeText(EveryProjectActivity.this, "视频信息加载失败，请重新播放(error code " + PolyvPlayErrorReason.VIDEO_NULL + ")", Toast.LENGTH_LONG).show();
                        break;
                    case PolyvPlayErrorReason.MP4_LINK_NUM_ERROR:
                        Toast.makeText(EveryProjectActivity.this, "MP4 播放地址服务器数据错误(error code " + PolyvPlayErrorReason.MP4_LINK_NUM_ERROR + ")", Toast.LENGTH_LONG).show();
                        break;
                    case PolyvPlayErrorReason.M3U8_LINK_NUM_ERROR:
                        Toast.makeText(EveryProjectActivity.this, "HLS 播放地址服务器数据错误(error code " + PolyvPlayErrorReason.M3U8_LINK_NUM_ERROR + ")", Toast.LENGTH_LONG).show();
                        break;
                    case PolyvPlayErrorReason.HLS_SPEED_TYPE_NULL:
                        Toast.makeText(EveryProjectActivity.this, "请设置播放速度(error code " + PolyvPlayErrorReason.HLS_SPEED_TYPE_NULL + ")", Toast.LENGTH_LONG).show();
                        break;
                    case PolyvPlayErrorReason.NOT_LOCAL_VIDEO:
                        Toast.makeText(EveryProjectActivity.this, "找不到本地下载的视频文件，请连网后重新下载(error code " + PolyvPlayErrorReason.NOT_LOCAL_VIDEO + ")", Toast.LENGTH_LONG).show();
                        break;
                    case PolyvPlayErrorReason.HLS_15X_INDEX_EMPTY:
                        Toast.makeText(EveryProjectActivity.this, "视频不支持1.5倍自动清晰度播放(error code " + PolyvPlayErrorReason.HLS_15X_INDEX_EMPTY + ")", Toast.LENGTH_LONG).show();
                        break;
                    case PolyvPlayErrorReason.HLS_15X_ERROR:
                        Toast.makeText(EveryProjectActivity.this, "视频不支持1.5倍当前清晰度播放(error code " + PolyvPlayErrorReason.HLS_15X_ERROR + ")", Toast.LENGTH_LONG).show();
                        break;
                    case PolyvPlayErrorReason.HLS_15X_URL_ERROR:
                        Toast.makeText(EveryProjectActivity.this, "1.5倍当前清晰度视频正在编码中(error code " + PolyvPlayErrorReason.HLS_15X_URL_ERROR + ")", Toast.LENGTH_LONG).show();
                        break;
                    case PolyvPlayErrorReason.M3U8_15X_LINK_NUM_ERROR:
                        Toast.makeText(EveryProjectActivity.this, "HLS 1.5倍播放地址服务器数据错误(error code " + PolyvPlayErrorReason.M3U8_15X_LINK_NUM_ERROR + ")", Toast.LENGTH_LONG).show();
                        break;
                    case PolyvPlayErrorReason.CHANGE_EQUAL_BITRATE:
                        Toast.makeText(EveryProjectActivity.this, "切换清晰度相同，请选择其它清晰度(error code " + PolyvPlayErrorReason.CHANGE_EQUAL_BITRATE + ")", Toast.LENGTH_LONG).show();
                        break;
                    case PolyvPlayErrorReason.CHANGE_EQUAL_HLS_SPEED:
                        Toast.makeText(EveryProjectActivity.this, "切换播放速度相同，请选择其它播放速度(error code " + PolyvPlayErrorReason.CHANGE_EQUAL_HLS_SPEED + ")", Toast.LENGTH_LONG).show();
                        break;
                    case PolyvPlayErrorReason.CAN_NOT_CHANGE_BITRATE:
                        Toast.makeText(EveryProjectActivity.this, "未开始播放视频不能切换清晰度，请先播放视频(error code " + PolyvPlayErrorReason.CAN_NOT_CHANGE_BITRATE + ")", Toast.LENGTH_LONG).show();
                        break;
                    case PolyvPlayErrorReason.CAN_NOT_CHANGE_HLS_SPEED:
                        Toast.makeText(EveryProjectActivity.this, "未开始播放视频不能切换播放速度，请先播放视频(error code " + PolyvPlayErrorReason.CAN_NOT_CHANGE_HLS_SPEED + ")", Toast.LENGTH_LONG).show();
                        break;
                    case PolyvPlayErrorReason.QUESTION_ERROR:
                        Toast.makeText(EveryProjectActivity.this, "视频问答数据加载失败，请重新播放(error code " + PolyvPlayErrorReason.QUESTION_ERROR + ")", Toast.LENGTH_LONG).show();
                        break;
                    case PolyvPlayErrorReason.CHANGE_BITRATE_NOT_EXIST:
                        Toast.makeText(EveryProjectActivity.this, "视频没有这个清晰度，请切换其它清晰度(error code " + PolyvPlayErrorReason.CHANGE_BITRATE_NOT_EXIST + ")", Toast.LENGTH_LONG).show();
                        break;
                    case PolyvPlayErrorReason.HLS_URL_ERROR:
                        Toast.makeText(EveryProjectActivity.this, "播放地址异常，无法播放(error code " + PolyvPlayErrorReason.HLS_URL_ERROR + ")", Toast.LENGTH_LONG).show();
                        break;
                    case PolyvPlayErrorReason.LOADING_VIDEO_ERROR:
                        Toast.makeText(EveryProjectActivity.this, "视频信息加载中出现异常，请重新播放(error code " + PolyvPlayErrorReason.LOADING_VIDEO_ERROR + ")", Toast.LENGTH_LONG).show();
                        break;
                    case PolyvPlayErrorReason.HLS2_URL_ERROR:
                        Toast.makeText(EveryProjectActivity.this, "播放地址异常，无法播放(error code " + PolyvPlayErrorReason.HLS2_URL_ERROR + ")", Toast.LENGTH_LONG).show();
                        break;
                    case PolyvPlayErrorReason.TOKEN_NULL:
                        Toast.makeText(EveryProjectActivity.this, "播放授权获取失败，请重新播放(error code " + PolyvPlayErrorReason.TOKEN_NULL + ")", Toast.LENGTH_LONG).show();
                        break;
                }
        ...
        videoView.setOnErrorListener(new IPolyvOnErrorListener2() {
            ...
            public boolean onError() {
                Toast.makeText(EveryProjectActivity.this, "视频异常，请重新播放", Toast.LENGTH_SHORT).show();
                return true;
            }
        });
...
//        viewPagerFragment.getTalkFragment().onActivityResult(requestCode, resultCode, data);
    ...
    public static Intent newIntent(Context context, EveryProjectActivity.PlayMode playMode, String vid) {
        return newIntent(context, playMode, vid, PolyvBitRate.ziDong.getNum());
    }

    public static Intent newIntent(Context context, EveryProjectActivity.PlayMode playMode, String vid, int bitrate) {
        return newIntent(context, playMode, vid, bitrate, false);
    }

    public static Intent newIntent(Context context, EveryProjectActivity.PlayMode playMode, String vid, int bitrate, boolean startNow) {
        return newIntent(context, playMode, vid, bitrate, startNow, false);
    }

    public static Intent newIntent(Context context, EveryProjectActivity.PlayMode playMode, String vid, int bitrate, boolean startNow,
                                   boolean isMustFromLocal) {
        Intent intent = new Intent(context, EveryProjectActivity.class);
    ...
    public static void intentTo(Context context, EveryProjectActivity.PlayMode playMode, String vid) {
        intentTo(context, playMode, vid, PolyvBitRate.ziDong.getNum());
    }

    public static void intentTo(Context context, EveryProjectActivity.PlayMode playMode, String vid, int bitrate) {
        intentTo(context, playMode, vid, bitrate, false);
    }

    public static void intentTo(Context context, EveryProjectActivity.PlayMode playMode, String vid, int bitrate, boolean startNow) {
    ...
    public enum PlayMode {
        ...
        public static EveryProjectActivity.PlayMode getPlayMode(int code) {
            switch (code) {
                case 3:
                    return landScape;
                case 4:
