...
public  class Path {
    ...
    public static String CENTER(String uid,String token){
        String path="http://2.huobox.com/index.php?m=pcdata.vip_course_data&pc=1&uid="+uid+"&token="+token;
        return path;
    }
    //分类
    ...
    public static String MYCLASS(String uid,String token){
        String path="http://2.huobox.com/index.php?m=pcdata.course_data&pc=1&uid="+uid+"&token="+token;
        return path;
    }
    //图片
    public static String IMG(String img){
        String path="http://2.huobox.com"+img;
        return path;
    }
    //一级标题
    ...
    public static String SECOND(String sid,String uid,String token) {
        String path = "http://2.huobox.com/index.php?m=pcdata.vip_course2_data&pc=1&sid="+sid+"&uid=" + uid + "&token=" + token;
        return path;
    }
    //评论
    public static String COMMENT(String spid,String uid,String token) {
        String path = "http://2.huobox.com/index.php?m=pcdata.question&pc=1&spid=" + spid + "&uid=" + uid + "&token=" + token;
        return path;
    }
}
